...
def printEnergies(ffs, soft_ffs):
    ...
    for i in range(0,11):
        ...
        print "COUL: %8.5f " % alpha,

        for j in range(0,4):
            print "%9.3f " % ffs[j].energy( ffs[j].components().coulomb() ).to(kcal_per_mol),

        for j in range(0,4):
            print "%9.3f " % soft_ffs[j].energy( soft_ffs[j].components().coulomb() ).to(kcal_per_mol),

        print "\nLJ  : %8.5f " % alpha,

        for j in range(0,4):
            print "%9.3f " % ffs[j].energy( ffs[j].components().lj() ).to(kcal_per_mol),
                
        for j in range(0,4):
            print "%9.3f " % soft_ffs[j].energy( soft_ffs[j].components().lj() ).to(kcal_per_mol),
...
for i in range(0,residues.count() / 2):
    for j in range(0,4):
        ffs[j].add(residues[i], MGIdx(0))
        soft_ffs[j].add(residues[i], MGIdx(0))

for i in range(residues.count() / 2, residues.count()):
    for j in range(0,4):
