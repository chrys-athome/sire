...
ZmatrixLineTemplate::ZmatrixLineTemplate(const QString &atom , const QString &bond ,
        const QString &angle ,
...
bool ZmatrixTemplate::containsZmatrixLineTemplate(const QString &atom) const
{
    return this->zmatrix.contains(atom);
}
...
void ZmatrixTemplate::addZmatrixLineTemplate( const QString &atom,
        const ZmatrixLineTemplate &zmatline)
{
...
void ZmatrixTemplate::setAngleDelta( const QString &atom, const QString &bond, const QString &angle,
                                     double angleDelta )
{
    if ( not this->zmatrix.contains(atom) )
        throw SireError::invalid_key(atom, CODELOC);
...
Molecule ZmatrixMaker::applyTemplates( Molecule &molecule)
{
    ...
    for (ResIdx i(0) ; i < nres ; ++i)
    {
        ...
        for (int i=0; i<atoms.count(); ++i)
        {
            ...
            if (restemplate.containsZmatrixLineTemplate( atom.name().value() ))
            {
