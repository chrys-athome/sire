...
QHash<QString,double> createLengthUnits()
{
    ...
    double megameter = 1000 * kilometer;
    ...
    double bohr_radii = 0.52917720859 * angstrom;
...
static QHash<QString,double> *length_scale_factors(0);

void createLengthScaleFactors()
{
    while (length_scale_factors == 0)
    {
        QHash<QString,double> *scl = new QHash<QString,double>( createLengthUnits() );
        
        //this should use an atomic pointer...!
        if (length_scale_factors == 0)
            length_scale_factors = scl;
        else
            delete scl;
    }
}

double LengthValue::scaleFactor(QString &units)
{
    units = units.trimmed();
    ...
    createLengthScaleFactors();

    double scl = length_scale_factors->value(units,0);
    
    if (scl == 0)
    {
        throw SireError::invalid_arg( QObject::tr(
                "Cannot interpret the length unit \"%1\". Available length units "
                "are [ %2 ].")
                    .arg(units, QStringList(length_scale_factors->keys()).join("\n")),
                        CODELOC );
    }
...
LengthValue::LengthValue(double value, QString units) 
             : Value(), val(value), minval(0), maxval(0),
               has_minval(false), has_maxval(false)
{
    sclfac = scaleFactor(units);
    unitname = units;
}

/** Construct an LengthValue with passed value, and specified minimum */
LengthValue LengthValue::valueWithMinimum(double value, double minimum, QString units)
{
    LengthValue v(value, units);
    v.minval = minimum;
    v.has_minval = true;
...
/** Construct an LengthValue with passed value, and specified maximum */
LengthValue LengthValue::valueWithMaximum(double value, double maximum, QString units)
{
    LengthValue v(value, units);
    v.maxval = maximum;
    v.has_maxval = true;
...
/** Construct an LengthValue with passed value and specified range */
LengthValue LengthValue::valueWithRange(double value, double minimum, double maximum,
                                        QString units)
{
    LengthValue v(value, units);
...
QDomElement LengthValue::toDomElement(QDomDocument doc) const
{
    QDomElement elem = doc.createElement("length");
    elem.appendChild( doc.createTextNode( QString::number(val) ) );
...
LengthValue::LengthValue(QString value)
           : Value(), minval(0), maxval(0),
             has_minval(false), has_maxval(false)
{
    QPair<double,QString> parsed_value = readDimension(value);
    this->operator=( LengthValue(parsed_value.first, parsed_value.second) );
}

/** Construct from a QDomElement */
LengthValue::LengthValue(QDomElement elem) 
           : Value(), sclfac(1), val(0), minval(0), maxval(0),
             has_minval(false), has_maxval(false)
{
    if (elem.tagName() != "length")
        throw SireError::file_error( QObject::tr(
                "Can only create a LengthValue object from a <length>...</length> "
                "XML DOM element. Cannot use a <%1>...</%1> element!")
                    .arg(elem.tagName()), CODELOC );
...
/** Copy constructor */
LengthValue::LengthValue(const LengthValue &other)
           : Value(other), 
...
LengthValue* LengthValue::ptr_clone() const
{
    return new LengthValue(*this);
}

/** Copy assignment operator */
LengthValue& LengthValue::operator=(const LengthValue &other)
{
    if (this != &other)
    {
        unitname = other.unitname;
...
/** Comparison operator */
bool LengthValue::operator==(const LengthValue &other) const
{
...
bool LengthValue::operator!=(const LengthValue &other) const
{
    return not LengthValue::operator==(other);
}

const char* LengthValue::typeName()
{
    return "SireSim::LengthValue";
}

const char* LengthValue::what() const
{
    return LengthValue::typeName();
}

/** Return the actual value, with no conversion from the units
    in which this value was supplied */
double LengthValue::value() const
{
    return val;
}

/** Return the actual value, converted to units 'units' */
double LengthValue::value(QString units) const
{
    double sclto = scaleFactor(units);
    
    return val * (sclfac/sclto);
}

QString LengthValue::toValueString() const
{
    if (unitname.isEmpty())
        return QString::number(val, 'g', 12);
    else
...
ValuePtr LengthValue::fromValueString(QString value) const
{
    QPair<double,QString> parsed_value = readDimension(value);
    
    double sclfrom = scaleFactor(parsed_value.second);
