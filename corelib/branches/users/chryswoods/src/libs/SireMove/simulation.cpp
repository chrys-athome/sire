...
/** Constructor */
LocalSim::LocalSim() : SimHandle(), sim_mutex( QMutex::Recursive ),
...
LocalSim::~LocalSim()
{
    //don't let a running simulation be destroyed!
    sim_mutex.lock();
    
    //ok - now we know that the simulation has finished
    sim_mutex.unlock();
}
