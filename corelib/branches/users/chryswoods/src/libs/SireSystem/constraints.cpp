...
{
    writeHeader(ds, r_constraints, 1);
...
{
    return cons.at( Index(i).map(cons.count()) ).read();
}
...
/** Return iterator pointing to the first constraint in this set */
Constraints::const_iterator Constraints::constBegin() const
{
    return cons.constBegin();
}

/** Return iterator pointing just past the constraint in this set */
Constraints::const_iterator Constraints::constEnd() const
{
    return cons.constEnd();
}
...
{
    QMutableListIterator<ConstraintPtr> it(cons);
...
bool Constraints::areSatisfied(System &system) const
{
    for (Constraints::const_iterator it = this->constBegin();
         it != this->constEnd();
         ++it)
    {
...
*/
void Constraints::assertSatisfied(System &system) const
{
...
void Constraints::apply(System &system) const
{
