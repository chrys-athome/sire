...
#include "third_party/MersenneTwister.h"       // CONDITIONAL_INCLUDE
...
static const RegisterObject<RanGenerator> r_rangen;
...
RanGenerator::RanGenerator() : Implements<RanGenerator,Object>(), d(shared_null)
{}

/** Create a generator seeded with 'seed' */
RanGenerator::RanGenerator(quint32 seed)
             : Implements<RanGenerator,Object>(), d( new RanGeneratorPvt(seed) )
{}
...
RanGenerator::RanGenerator(const RanGenerator &other)
             : Implements<RanGenerator,Object>(other), d( other.d )
{}
...
void RanGenerator::stream(Stream &s)
{
    ...
    SharedSchema shared = s.shared(d);
    
    if (shared.mustStream())
    {
        QVector<quint32> state;
        const int state_size = MTRand::N + 1;
        
        if (s.isSaving())
            state = this->getState();

        ArraySchema schema = s.array<quint32>(state_size);
        
        for (int i=0; i<state_size; ++i)
        {
            schema.index(i) & state[i];
        }
