...
void OpenMMFrEnergyST::initialise()  {
	...
	if ( flag_cutoff == NOCUTOFF ){
        ...
        custom_intra_14_todummy = new OpenMM::CustomBondForce("(10.0^n) * Hcs + Hls;"
                                                              "Hcs=((0.1*lam)^(n))*138.935456*q_prod/sqrt(diff_cl+r^2);"
        ...
        custom_intra_14_fromdummy = new OpenMM::CustomBondForce("(10.0^n) * Hcs + Hls;"
                                                                "Hcs=((0.1*lam)^(n))*138.935456*q_prod/sqrt(diff_cl+r^2);"
                                                                "diff_cl=(1.0-lam)*0.01;"
                                                                "Hls=4.0*eps_avg*(LJ*LJ-LJ);"
                                                                "LJ=((sigma_avg*sigma_avg)/soft)^3;"
        ...
        custom_intra_14_fromdummy_todummy = new OpenMM::CustomBondForce("(10.0^n) * Hcs + Hls;"
                                                                        "Hcs=((0.1*lam)^(n))*138.935456*q_prod/sqrt(diff_cl+r^2);"
                                                                        "diff_cl=(1.0-lam)*0.01;"
                                                                        "Hls=4.0*eps_avg*(LJ*LJ-LJ);"
                                                                        "LJ=((sigma_avg*sigma_avg)/soft)^3;"
...
/***********************************************************************BONDED INTERACTIONS****************************************************************/
	...
	for (int i=0; i < nmols ; i++){
		...
		}catch(int e){cout << "Solvent Molecule";}
		...
		for (int j=0; j < bonds_ff.length() ; j++){
			...
			if(molecule.isSameMolecule(solutemol)){
                ...
                if(bond_pert_list.indexOf(bond_ff) != -1 || bond_pert_swap_list.indexOf(bond_ff) != -1 ){//Solute molecule. Check if the current solute bond is in the perturbed bond list
                    ...
                    //******** ATTENTION   TO BE ADDED  bondStretch_openmm->addBond(idx0,idx1,r0 * OpenMM::NmPerAngstrom, k * 2.0 * OpenMM::KJPerKcal * OpenMM::AngstromsPerNm * OpenMM::AngstromsPerNm);
                    bondPairs.push_back(std::make_pair(idx0,idx1));
                    continue;
                }
