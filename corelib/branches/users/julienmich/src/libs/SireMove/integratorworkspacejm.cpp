...
QDataStream SIREMOVE_EXPORT &operator>>(QDataStream &ds, IntegratorWorkspaceJM &intws)
{
    VersionID v = readHeader(ds, r_intwsjm);
...
void IntegratorWorkspaceJM::setPropertyMap(const PropertyMap &m)
{
    map = m;
}
...
void IntegratorWorkspaceJM::setSpaceProperty(const PropertyName &source)
{
    if (map["space"] != source)
    {
...
void IntegratorWorkspaceJM::setVelocitiesProperty(const PropertyName &source)
{
...
void IntegratorWorkspaceJM::setMassesProperty(const PropertyName &source)
{
    if (map["mass"] != source)
    {
        map.set("mass", source);
...
void IntegratorWorkspaceJM::setElementsProperty(const PropertyName &source)
{
...
void IntegratorWorkspaceJM::setVelocityGeneratorProperty(const PropertyName &source)
{
...
PropertyName IntegratorWorkspaceJM::coordinatesProperty() const
{
    return map["coordinates"];
}

/** Return the property that contains the system space */
PropertyName IntegratorWorkspaceJM::spaceProperty() const
{
    return map["space"];
}

/** Return the property that contains the molecule velocities */
PropertyName IntegratorWorkspaceJM::velocitiesProperty() const
{
    return map["velocity"];
}

/** Return the property that contains the molecule masses */
PropertyName IntegratorWorkspaceJM::massesProperty() const
{
    return map["mass"];
}

/** Return the property that contains the molecule elements */
PropertyName IntegratorWorkspaceJM::elementsProperty() const
{
    return map["element"];
}

/** Return the property used to generate missing velocities */
PropertyName IntegratorWorkspaceJM::velocityGeneratorProperty() const
{
...
bool IntegratorWorkspaceJM::calculateForces(const Symbol &nrg_component)
{
...
bool IntegratorWorkspaceJM::forcesNeedCalculating(const Symbol &nrg_component) const
{
...
void IntegratorWorkspaceJM::mustNowRecalculateFromScratch()
{
    need_new_forces = true;
    last_nrg_component = Symbol();
}
...
/** Internal function used to update the system and molecule group with
    changed molecules */
void IntegratorWorkspaceJM::pvt_update(const Molecules &changed_mols)
{
...
void IntegratorWorkspaceJM::collectStatistics()
{
    sys.collectStats();
}
...
//////////
////////// Implementation of NullIntegratorWorkspaceJM
//////////
                                        ...
                                        AtomicVelocityWorkspaceJM &atvelws)
{
    ...
    if (v == 1)
    {
        ...
        throw version_error(v, "1", r_atvelwsjm, CODELOC);
...
void AtomicVelocityWorkspaceJM::rebuildFromScratch()
{
    ...
    const EnergyTable &energytable = this->energyTable();
...
bool AtomicVelocityWorkspaceJM::calculateForces(const Symbol &nrg_component)
{
    if (not IntegratorWorkspaceJM::calculateForces(nrg_component))
        return false;
...
/** Regenerate the velocities using the passed generator */
void AtomicVelocityWorkspaceJM::regenerateVelocities(const VelocityGenerator &generator)
{
    const MoleculeGroup &molgroup = moleculeGroup();
...
/** Comparison operator */
bool AtomicVelocityWorkspaceJM::operator==(const AtomicVelocityWorkspaceJM &other) const
{
    return IntegratorWorkspaceJM::operator==(other);
}
...
/** Return the total kinetic energy of the molecule with number 'molnum'
    ...
    \throw SireMol::missing_molecule
*/
MolarEnergy AtomicVelocityWorkspaceJM::kineticEnergy(MolNum molnum) const
{
...
int AtomicVelocityWorkspaceJM::nMolecules() const
{
...
int AtomicVelocityWorkspaceJM::nAtoms(int i) const
{
...
bool AtomicVelocityWorkspaceJM::setSystem(const System &new_system)
{
    if (IntegratorWorkspaceJM::setSystem(new_system))
    {
        this->rebuildFromScratch();
        return true;
    }
...
void AtomicVelocityWorkspaceJM::commitVelocities()
{
    int nmols = atom_coords.count();
...
void AtomicVelocityWorkspaceJM::commitCoordinatesAndVelocities()
{
    int nmols = atom_coords.count();
