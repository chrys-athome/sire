...
inline QDataStream& writeHeader(QDataStream &ds, const RegisterMetaTypeBase &r_type, 
                                VersionID version)
{
...
/** Write a header to the data stream that contains the magic and version  
    of the object that is about to be written */
inline QDataStream& writeHeader(QDataStream &ds, MagicID magicid, VersionID version)
{
    ds << magicid << version;
    return ds;
}
...
inline VersionID readHeader(QDataStream &ds, const RegisterMetaTypeBase &r_type)
{
    MagicID id;
    VersionID v;
...
/** Read the header of the binary object to check that the type is correct 
    and to obtain the binary data version */
inline VersionID readHeader(QDataStream &ds, MagicID magicid, const char *type_name)
{
    MagicID id;
    VersionID v;
