...
void TestAngle::runTests()
{
    ...
    for (int i=10; i>=-10; --i)
    {
        ...
        BOOST_CHECK( testEqual( cang.toDegrees(), degs ) );
    ...
    BOOST_CHECK( testEqual( Angle::degrees(35.4343).toDegrees(), 35.4343 ) );
    
    ang = Angle::degrees(45.0);
    BOOST_CHECK( testEqual( ang.small().toDegrees(), Angle::degrees(45.0).toDegrees() ) );
    BOOST_CHECK( testEqual( ang.large().toDegrees(), Angle::degrees(-315.0).toDegrees() ) );
    BOOST_CHECK( testEqual( ang.positive().toDegrees(), Angle::degrees(45.0).toDegrees() ) );
    BOOST_CHECK( testEqual( ang.negative().toDegrees(), Angle::degrees(-315.0).toDegrees() ) );
