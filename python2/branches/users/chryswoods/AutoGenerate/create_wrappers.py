...
def _generate_bases(self, base_creators):
    ...
    for base_desc in self.declaration.bases:
        ...
        if (base_desc.related_class.demangled.replace(" ","") in all_exposed_classes):
...
def find_class(mb, classname):
   #remove all spaces (as these confuse things!)
   classname = classname.replace(" ","")

   for clas in mb.classes():
       clasname = str(clas)

       clasname = clasname.split("[class]")[0].split("[struct]")[0]
       clasname = clasname.replace(" ","")
       
       alias = clas.alias
       alias = alias.replace(" ","")

       #print "%s == %s?  [%s]" % (classname, clasname, alias)

       if clasname == classname:
           return clas
       elif alias == classname:
           return clas
...
def export_class(mb, classname, aliases, includes, special_code, auto_str_function=True):
   ...
   if auto_str_function:
       if has_function(c, "toString"):
           ...
           c.add_declaration_code( "#include \"Siren/str.hpp\"" )
   ...
   if (classname in aliases):
      c.alias = aliases[classname].split("::")[-1]
