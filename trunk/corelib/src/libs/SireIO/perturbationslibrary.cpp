...
void PerturbationsTemplate::setInitCharge(const QString &atomname,
        const SireUnits::Dimension::Charge &atomcharge)
{
    initcharges.insert(atomname, atomcharge);
}

void PerturbationsTemplate::setFinalCharge(const QString &atomname,
        const SireUnits::Dimension::Charge &atomcharge)
{
    finalcharges.insert(atomname, atomcharge);
}

bool PerturbationsTemplate::containsInitCharge(const QString &atomname) const
{
    return initcharges.contains(atomname);
}
...
bool PerturbationsTemplate::containsFinalCharge(const QString &atomname) const
{
    return finalcharges.contains(atomname);
}
...
bool PerturbationsTemplate::containsInitLJ(const QString &atomname) const
{
    return initLJs.contains(atomname);
}
...
bool PerturbationsTemplate::containsFinalLJ(const QString &atomname) const
{
    return finalLJs.contains(atomname);
}
...
bool PerturbationsTemplate::containsInitType(const QString &atomname) const
{
    return initatypes.contains(atomname);
}
...
bool PerturbationsTemplate::containsFinalType(const QString &atomname) const
{
    return finalatypes.contains(atomname);
}
...
bool PerturbationsTemplate::containsInitBondK(const BondID &bond) const
{
    return initbondsk.contains(bond);
}
...
bool PerturbationsTemplate::containsInitBondR(const BondID &bond) const
{
    return initbondsr.contains(bond);
}
...
bool PerturbationsTemplate::containsFinalBondK(const BondID &bond) const
{
    return finalbondsk.contains(bond);
}
...
bool PerturbationsTemplate::containsFinalBondR(const BondID &bond) const
{
    return finalbondsr.contains(bond);
}
...
bool PerturbationsTemplate::containsInitAngleK(const AngleID &angle) const
{
    return initanglesk.contains(angle);
}
...
bool PerturbationsTemplate::containsInitAngleT(const AngleID &angle) const
{
    return initanglest.contains(angle);
}
...
bool PerturbationsTemplate::containsFinalAngleK(const AngleID &angle) const
{
    return finalanglesk.contains(angle);
}
...
bool PerturbationsTemplate::containsFinalAngleT(const AngleID &angle) const
{
    return finalanglest.contains(angle);
}
...
void PerturbationsTemplate::setInitDihParams(const DihedralID &dihedral,
...
bool PerturbationsTemplate::containsInitDihParams(const DihedralID &dihedral) const
{
    return initdihparams.contains(dihedral);
}
...
void PerturbationsTemplate::setFinalDihParams(const DihedralID &dihedral,
        const QList<double> &params)
{
...
bool PerturbationsTemplate::containsFinalDihParams(const DihedralID &dihedral) const
{
    return finaldihparams.contains(dihedral);
}
...
void PerturbationsTemplate::setInitImpParams(const ImproperID &improper,
        const QList<double> &params)
{
    initimpparams.insert(improper, params);
}

bool PerturbationsTemplate::containsInitImpParams(const ImproperID &improper) const
{
    return initimpparams.contains(improper);
}
...
void PerturbationsTemplate::setFinalImpParams(const ImproperID &improper,
        const QList<double> &params)
{
    finalimpparams.insert(improper, params);
}

bool PerturbationsTemplate::containsFinalImpParams(const ImproperID &improper) const
{
    return finalimpparams.contains(improper);
}
...
void PerturbationsLibrary::loadTemplates(const QString &templatefile)
{
    ...
    while ( not line.isNull() )
    {
        ...
        {
            DihedralID dihedral = DihedralID( AtomName(atom0), AtomName(atom1), AtomName(atom2),
                                              AtomName(atom3) );
        ...
        {
            ImproperID improper = ImproperID( AtomName(atom0), AtomName(atom1), AtomName(atom2),
                                              AtomName(atom3) );
...
Molecule PerturbationsLibrary::applyTemplate(const Molecule &molecule) const
{
    ...
    if ( not templates.contains(molname) )
        throw SireError::invalid_key(
            QObject::tr("There is no perturbations template for the molecule with name %1"
    ...
    for (int i=0; i<nats; ++i)
    {
        ...
        //qDebug() << atom.name();
        // the charges...
        if (pert.containsInitCharge(iname))
        {
            atom.setProperty( initial_charge_property, pert.getInitCharge(iname) );
        }

        if (pert.containsFinalCharge(iname))
        {
            atom.setProperty( final_charge_property, pert.getFinalCharge(iname) );
        }

        // the LJ parameters...
        if (pert.containsInitLJ(iname))
        {
            atom.setProperty(initial_LJ_property, pert.getInitLJ(iname));
        }

        if (pert.containsFinalLJ(iname))
        {
            atom.setProperty(final_LJ_property, pert.getFinalLJ(iname));
        }

        // the amber types...
        if (pert.containsInitType(iname))
        {
            atom.setProperty(initial_ambertype_property, pert.getInitType(iname));
        }

        if (pert.containsFinalType(iname))
        {
            atom.setProperty(final_ambertype_property, pert.getFinalType(iname));
        }
