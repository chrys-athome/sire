...
/*        BOOST_CHECK( testEqual( Cosh(x).evaluate(x==xval), std::cosh(xval) ) );
    ...
    /** See if we can extract all of the Cos or Sin functions from an expression */
    ex1 = Cos(x) + Sin(2*x) + Cos(3*x) + Sin(4*x) + Cos(5*x);

    qDebug() << ex1.toString();

    QList<Cos> cosfuncs = ex1.children<Cos>();
    QList<Sin> sinfuncs = ex1.children<Sin>();

    BOOST_CHECK_EQUAL( cosfuncs.count(), 3 );
    BOOST_CHECK_EQUAL( sinfuncs.count(), 2 );

    ex2 = 0;
    foreach (Cos cosfunc, cosfuncs )
    {
        ex2 += cosfunc;
    }
    ...
    BOOST_CHECK( ex2 == Cos(x) + Cos(3*x) + Cos(5*x) );

    ex2 = 0;
    foreach( Sin sinfunc, sinfuncs )
    {
        ex2 += sinfunc;
    }
    ...
    BOOST_CHECK( ex2 == Sin(2*x) + Sin(4*x) );
