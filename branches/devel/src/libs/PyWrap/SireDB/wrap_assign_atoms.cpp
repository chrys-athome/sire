...
void wrap_assignParameters3( const AssignBase &assignbase, ParameterTable &param_table, 
                             ParameterDB &database,
                             const MatchMRData &matchmr )
{
    return assignbase.assignParameters(param_table, database, matchmr);
}

void wrap_assignParameters2( const AssignBase &assignbase, ParameterTable &param_table, 
                             ParameterDB &database )
{
    return assignbase.assignParameters(param_table, database);
}
