...
*/
ParameterTable assign_parameters::assign(const ParameterTable &orig_table, 
                                         ParameterDB &database,
                                         const MatchMRData &matchmr) const
{
    //create a copy of the original table - this prevent corrupting
    ...
    for (QList< DynamicSharedPtr<AssignBase> >::const_iterator it = instrctns.begin();
         ...
         ++it)
    {
        (*it)->assignParameters(new_table, database, matchmr);
    }
...
ParameterTable assign_parameters::assign(const Molecule &molecule, 
                                         ...
                                         const MatchMRData &matchmr) const
{
    return assign(ParameterTable(molecule), database, matchmr);
}
