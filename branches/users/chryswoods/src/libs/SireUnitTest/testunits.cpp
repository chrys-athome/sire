...
void TestUnits::runTests()
{
    ...
    qDebug() << QObject::tr("Gas constant = %1 (kcal mol-1 K-1)")
                    .arg(gasr);
    qDebug() << QObject::tr("One atmosphere = %1 (kcal mol-1 A-3)").arg(atm_per_mol);

    //test conversion of epsilon_0 from SI units to internal units
    BOOST_CHECK_CLOSE( convertFrom(8.854187817e-12, 
                                  (coulomb*coulomb) / (J_per_mol*meter)), 
                       epsilon0, 1e-6 );

    BOOST_CHECK_CLOSE( gasr, double(k_boltz * per_mole), 1e-6 );
    ...
    BOOST_CHECK_CLOSE( double(atm_per_mol), 1.45839756e-05, 1e-6 );
