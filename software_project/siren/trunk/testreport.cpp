...
{
    return String::tr("%1: (passed)").arg(this->description());
}
...
{
    return String::tr("%1: (failed!)").arg(this->description());
}
...
String TestFailedCompareItem::toString() const
{
    ...
    {
        return String::tr("%1: (failed! Objects of type %2 and %3 are not equal)")
                    .arg(this->description(),
                         obtained_val.what(),
                         expected_val.what());
    }
    ...
    {
        return String::tr("%1: (failed! Two objects of type %2 are not equal)")
                    .arg(this->description(),
                         obtained_val.what());
    }
}
...
TestFailedExceptionItem::TestFailedExceptionItem(const Exception &e)
                        : TestReportItem( String::tr("Caught unexpected exception!") ),
                          excptn(e)
{}
...
String TestFailedExceptionItem::toString() const
{
    ...
    {
        return String::tr("%1: (failed! Exception %1 was thrown)")
                        .arg(this->description(), 
...
String TestReport::toString() const
{
    if (this->isEmpty())
    {
        return String::tr("%1: (passed, but no tests were performed!)")
                            .arg(this->description());
    }
    else if (this->passed())
    {
    ...
    {
        return String::tr("%1: (failed! numberOfFailures() == %2)")
...
String TestReportEditor::toString() const
{
    if (this->isEmpty())
    {
        return String::tr("%1: (passed, but no tests were performed!) [EDIT]")
                            .arg(this->description());
    }
    else if (this->passed())
    {
        return String::tr("%1: (passed) [EDIT]").arg(this->description());
    }
    else
    {
        return String::tr("%1: (failed! numberOfFailures() == %2) [EDIT]")
                            .arg(this->description()).arg(this->numberOfFailures());
    }
}
