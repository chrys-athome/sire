...
static StringList getBackTrace()
{
    ...
    if (nfuncs == 1)
    {
        ...
        ret.append( Siren::tr("This is an incomplete backtrace as it looks "
...
void Exception::test(TestReportEditor &report) const
{
    ...
    catch(const Exception &e)
    {
        report.testTrue( e.isA(this->what()), 
                    Siren::tr("Exception type %1 is caught correctly.")
                                .arg(this->what()) );
                                
        report.testEqual( *this, e,
                    Siren::tr("Caught exception is equal to itself, from throwSelf()") );
    }
}
