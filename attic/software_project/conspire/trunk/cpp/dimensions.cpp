...
namespace Conspire
{
    namespace detail
    {
        Pair<String,int64>::Type CONSPIRE_EXPORT getPostFactor(String text)
        {
            text = text.trimmed();

            //process the text to see if any multiples are added
            //to the end of the string (e.g. million, thousand etc.)
            StringList words = text.split(" ", String::SkipEmptyParts);
            
            int64 postfac = 1;
            
            List<String>::MutableIterator it(words);
            ...
            return Pair<String,qint64>::Type(text,postfac);
        }
        
        int64 CONSPIRE_EXPORT readInt(String text)
        {
            text = text.trimmed();

            Pair<String,int64>::Type value = getPostFactor(text);
                                                                ...
                                                                StringList units)
        {
            ...
            if (value.isEmpty())
                return Pair<double,String>::Type(0,String::null);
        
            int index = -1;
        
            //we need to find the longest matching unit string
            foreach (const String &unit, units)
            {
            ...
            if (index == -1)
            {
                ...
                return Pair<double,String>::Type(val,String::null);
            }
            else
            {
                double val = readFloat(value.mid(0,index));
                return Pair<double,String>::Type(val,value.mid(index));
            }
        }

        bool readBool(String text)
        {
...
{
    return "Conspire::LengthValue";
}

String LengthData::tagName()
{
    static String tagname = "length";
    return tagname;
}

Hash<String,double>::Type *length_units(0);

Hash<String,double>::Type LengthData::units()
{
    while (length_units == 0)
    {
        Hash<String,double>::Type units;
        ...
        if (length_units == 0)
            length_units = new Hash<String,double>::Type(units);
    }
...
namespace Conspire
{
    REGISTER_SIREN_TEMPLATE_CLASS(Conspire::LengthValue)
...
{
    return "Conspire::AreaValue";
}

String AreaData::tagName()
{
    static String tagname = "area";
    return tagname;
}

Hash<String,double>::Type *area_units(0);

Hash<String,double>::Type AreaData::units()
{
    while (area_units == 0)
    {
        Hash<String,double>::Type units;
        ...
        if (area_units == 0)
            area_units = new Hash<String,double>::Type(units);
    }
...
namespace Conspire
{ 
    REGISTER_SIREN_TEMPLATE_CLASS(Conspire::AreaValue);
...
{
    return "Conspire::PressureValue";
}

String PressureData::tagName()
{
    static QString tagname = "pressure";
    return tagname;
}

Hash<String,double>::Type *pressure_units(0);

Hash<String,double>::Type PressureData::units()
{
    while (pressure_units == 0)
    {
        ...
        if (pressure_units == 0)
            pressure_units = new Hash<String,double>::Type(units);
    }
...
namespace Conspire
{
    REGISTER_SIREN_TEMPLATE_CLASS(Conspire::PressureValue)
