...
REGISTER_SIREN_CLASS(Conspire::Range)

/** Construct a null range */
Range::Range() : Object(), _start(0), _end(0), _step(0)
{}

/** Construct a range that represents the single index 'index' */
Range::Range(int index) : Object(), _start(index), _end(index), _step(0)
{}
...
Range::Range(int start, int stop, int step)
      : Object(), _start(start), _end(stop), _step(step)
{
    if (_start == _end)
        _step = 0;
...
Range::Range(String range) : Object()
{
    //remove all spaces and everything from the ends of the string
    String parsed = range.trimmed().replace(" ","");
    ...
    StringList words = parsed.split("..", String::KeepEmptyParts);
...
Range::Range(const Range &other)
      : Object(), _start(other._start), _end(other._end), _step(other._step)
{}
...
void Range::copy_object(const Range &other)
{
    if (this != &other)
    {
        _start = other._start;
...
/** Comparison operator */
bool Range::compare_object(const Range &other) const
{
    return _start == other._start and _end == other._end 
              and _step == other._step and super::compare_object(other);
}
...
Pair<int,int>::Type getRange(int start, int end, int nvalues)
{
    if (start < 0)
    ...
    return Pair<int,int>::Type(start,end);
}
...
bool Range::contains(int i, int nvalues) const
{
    ...
    Pair<int,int>::Type range = getRange(_start,_end,nvalues);
...
List<int>::Type Range::indicies(int nvalues) const
{
...
List<int>::Type Range::indicies(int i, int nvalues, int batchsize) const
{
    if (_start == 0 or _end == 0)
        return List<int>::Type();
...
REGISTER_SIREN_CLASS(Conspire::IndexList)

void cleanList(List<Range>::Type &ranges)
{
    List<Range>::MutableIterator it(ranges);
...
IndexList::IndexList(List<int>::Type indicies, bool allow_dups)
...
IndexList::IndexList(List<Range>::Type indices, bool allow_dups)
          : Value(), allow_duplicates(allow_dups)
{
...
IndexList::IndexList(QString indicies, bool allow_dups)
          : Value(), allow_duplicates(allow_dups)
{
    QStringList words = removeBrackets(indicies).split(",", String::SkipEmptyParts);
...
IndexList::IndexList(DomElement elem) : Value(), allow_duplicates(true)
{
...
DomElement IndexList::toDomElement(DomDocument doc) const
{
    DomElement elem = doc.createElement("index_list");
...
String IndexList::toValueString() const
{
    if (idxs.isEmpty())
        return "none";
    ...
    else
    {
        ...
        return String("[ %1 ]").arg(parts.join(", "));
    }
}

Obj IndexList::fromValueString(String value) const
{
...
/** Copy assignment operator */
void IndexList::copy_object(const IndexList &other)
{
    if (this != &other)
    {
        idxs = other.idxs;
...
/** Comparison operator */
bool IndexList::compare_object(const IndexList &other) const
{
...
/** Return the complete set of indicies, given a container containing
    'nvalues' objects */
List<int>::Type IndexList::indicies(int nvalues) const
{
    List<int>::Type ret;
    ...
    else
    {
        ...
        {
            List<int>::Type idxs_i = idxs.at(i).indicies(nvalues);
            
            if (i > 0)
            {
                List<int>::MutableIterator it(idxs_i);
...
List<int>::Type IndexList::indicies(int i, int nvalues, int batchsize) const
{
    return List<int>::Type();
}
